...
import { loadContinentGeodata, loadCountryGeodata } from '@/ressources/countryUtils';
import { ContinentCode } from '@/ressources/types';

type MapType = 'map' | 'glob';

type MapEntityType = 'country' | 'continent';
...
const MapV2 = <T,>({
	...
	useLayoutEffect(() => {
		...
		Promise.all(
			entities
				.map(async (entity) => {
					if (entity.type === 'country') return await loadCountryGeodata(entity.code, detailed);
					else if (entity.type === 'continent')
						return await loadContinentGeodata(entity.code as ContinentCode, detailed);
					else {
						return null;
					}
				})
			...
			entities.forEach((entity, index) => {
				const entityGeoData = geoData[index];
				if (!entityGeoData) return;

				const polygonSeries = chart.series.push(
					am5map.MapPolygonSeries.new(root, {
						geoJSON: entityGeoData,
					})
				);
			...
			// Gérer la rotation et le centrage
			if (center) {
